...
export const CartProvider = ({ children }) => {
  ...
  const putProductInCart = (product) => { // coloca os produtos que vem do menu no carrinho
    ...
    if (cartIndex >= 0) {
      // se retornar um numero maior ou igual a zero é pq já existe no carrinho
      newProductsInCart = cartProducts; // coloco os itens do cartProducts nessa variável

      newProductsInCart[cartIndex].quantity =
        newProductsInCart[cartIndex].quantity + 1; //pego o campo quantity, na exata posição do array e adiciono mais um a quantity
  ...
  const clearCart = () => { // esvaziar o carrinho
  ...
  const decreaseProduct = (productId) => {// enviando apenas o productId
    const cartIndex = cartProducts.findIndex((prd) => prd.id === productId);// acho index do produto no carrinho
 
    if(cartProducts[cartIndex].quantity > 1){// se a quantidade do item for maior q 1...
  ...
  const updateLocalStorage = (products) => { // função q salva o carrinho no localStorage
  ...
  useEffect(() => {
    ...
    if(clientCartData){ // se tiver alguma coisa dentro de clientCartData...
